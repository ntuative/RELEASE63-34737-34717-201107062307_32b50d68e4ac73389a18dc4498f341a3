package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1500:String = "asset-libraries";
      
      private static const const_1501:String = "library";
      
      private static const const_1505:String = "service-libraries";
      
      private static const const_1498:String = "library";
      
      private static const const_1502:String = "component-libraries";
      
      private static const const_1499:String = "library";
      
      private static const const_1506:String = "error_data";
      
      private static const const_1504:String = "error_cat";
      
      private static const const_1503:String = "error_desc";
       
      
      private var var_377:LibraryLoaderQueue;
      
      private var var_789:IEventDispatcher;
      
      private var var_531:uint;
      
      private var var_376:Function;
      
      private var var_213:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_275.push(new Array());
            this.var_631.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_428.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1249:
               debug("Core; using simple frame update handler");
               this.var_376 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_1006:
            ...
            case Core.const_797:
               ...
               this.var_213 = new Profiler(this);
               attachComponent(this.var_213,[new IIDProfiler()]);
               var_428.addChild(new ProfilerViewer(this.var_213));
               break;
            case Core.const_494:
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_275[i] as Array;
            ...
            if(var_428)
            {
               var_428.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_428 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1506,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1504,String(param3));
         ErrorReportStorage.setParameter(const_1503,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_789 = loadingEventDelegate;
         if(this.var_377 == null)
         {
            this.var_377 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1500)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_208,e);
         }
         try
         {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_208,e);
         }
         try
         {
            node = config.child(const_1502)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_208,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1194);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_789)
         {
            this.var_789.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_789)
         {
            ...
            this.var_789.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_789)
                  {
                     this.var_789.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_531 - this.var_377.length,this.var_531,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_213 ? 0 : this.var_1480 & 0;
         if(_loc3_ == Core.const_494)
         {
            this.var_275[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_275[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_213 ? 0 : this.var_1480 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_275[_loc5_] as Array;
            if(_loc4_ == Core.const_494)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_631[priority] = 0;
            receivers = this.var_275[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_428.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_631[priority] < priority)
               {
                  ++this.var_631[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_213.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
