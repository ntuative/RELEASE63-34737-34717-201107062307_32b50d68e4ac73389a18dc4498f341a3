package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1267:Number = 1.0;
      
      private var var_663:BitmapData;
      
      private var var_821:ColorTransform;
      
      private var var_1540:ColorTransform;
      
      private var var_1902:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_821 = new ColorTransform();
         this.var_821.alphaMultiplier = 1;
         this.var_1540 = new ColorTransform();
         this.var_1902 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_110 = this.var_1268[param1];
         this.var_1020 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1901 >= 0)
         {
            this.var_284 = this._visualization.getSprite(this.var_1901);
            if(this.var_284 && this.var_284.asset)
            {
               ...
               if(this.var_98 == null)
               {
                  this.var_98 = this.var_284.asset.clone();
                  if(this.var_821.alphaMultiplier != 1)
                  {
                     this.var_663 = new BitmapData(this.var_98.width,this.var_98.height,true,4278190080);
                  }
               }
               this.var_2587 = -this.var_284.offsetX;
               this.var_2588 = -this.var_284.offsetY;
               this.var_284.asset = this.var_98;
            }
            if(this.var_98)
            {
            ...
            if(this.var_663)
            {
               this.var_663.fillRect(this.var_663.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_110.y * this.var_1267;
         }
         return 0;
      }
      ...
      {
         if(!this.var_110 || !this.var_284)
         {
            return;
         }
         if(this.var_98 && this.var_284.asset != this.var_98)
         {
            this.var_284.asset = this.var_98;
         }
         if(this.var_1020)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1020 && this.var_110.hasIgnited)
         {
            this.var_1020 = true;
         }
         _loc8_ = this.var_1223 * this.var_1267;
         this.var_110.update();
         if(this.var_1020)
         {
            if(this.var_110.roomObjectSpriteId >= 0)
            ...
            {
               this.var_98.draw(this.var_663,this.var_1902,this.var_821,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_110.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2587 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1267;
               _loc4_ = this.var_2588 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1267;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1540.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_98.draw(_loc7_,this._translationMatrix,this.var_1540,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2589 = parseInt(param1.@size);
         this.var_1901 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1223 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1267 = this.var_2589 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_821.alphaMultiplier = this._blend;
