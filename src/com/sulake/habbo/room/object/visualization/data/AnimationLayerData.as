package com.sulake.habbo.room.object.visualization.data
{
   public class AnimationLayerData
   {
      ...
      public function dispose() : void
      {
         var _loc2_:* = null;
         var _loc1_:int = 0;
         while(_loc1_ < this.var_255.length)
         {
            _loc2_ = this.var_255[_loc1_] as AnimationFrameSequenceData;
            if(_loc2_ != null)
            {
               _loc2_.dispose();
            }
      ...
      public function calculateLength() : void
      {
         ...
         while(_loc1_ < this.var_255.length)
         {
            ...
            if(_loc2_ != null)
            {
               this.var_465 += _loc2_.frameCount;
            }
            _loc1_++;
         }
      }
      ...
      public function getFrame(param1:int, param2:int) : AnimationFrame
      {
         ...
         var _loc3_:* = null;
         param2 /= this.var_1995;
         var _loc4_:Boolean = false;
         var _loc5_:int = 0;
         if(!this.var_1996)
         {
            _loc6_ = param2 / this.var_465;
            param2 %= this.var_465;
            if(this._loopCount > 0 && _loc6_ >= this._loopCount || this._loopCount <= 0 && this.var_465 == 1)
            {
               param2 = this.var_465 - 1;
               _loc4_ = true;
            }
            _loc7_ = 0;
            _loc5_ = 0;
            while(_loc5_ < this.var_255.length)
            {
               _loc3_ = this.var_255[_loc5_] as AnimationFrameSequenceData;
               if(_loc3_ != null)
               {
         ...
         _loc5_ = this.var_255.length * Math.random();
         _loc3_ = this.var_255[_loc5_] as AnimationFrameSequenceData;
      ...
      {
         if(param2 < 0 || param2 >= this.var_255.length)
         {
            return null;
         }
         var _loc5_:AnimationFrameSequenceData = this.var_255[param2] as AnimationFrameSequenceData;
         if(_loc5_ != null)
         {
      ...
      private function getFrameFromSpecificSequence(param1:int, param2:AnimationFrameSequenceData, param3:int, param4:int, param5:Boolean) : AnimationFrame
      {
         ...
         if(param2 != null)
         {
            ...
            _loc13_ = this.var_1995 * _loc12_;
            if(param5)
            {
               _loc13_ = 0;
            }
            _loc14_ = false;
            if(!this.var_1996 && !param2.isRandom)
            {
               if(param3 == this.var_255.length - 1 && param4 == param2.frameCount - 1)
               {
                  _loc14_ = true;
               }
            }
