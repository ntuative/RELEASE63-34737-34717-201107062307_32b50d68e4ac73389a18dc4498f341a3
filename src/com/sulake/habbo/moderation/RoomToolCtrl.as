package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1174:IDropMenuWindow;
      
      private var var_528:ITextFieldWindow;
      
      private var var_705:Boolean = true;
      
      private var var_1106:ICheckBoxWindow;
      
      private var var_1647:ICheckBoxWindow;
      
      private var var_1646:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_35.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_385));
         Logger.log("BEGINNING TO SHOW: " + this.var_385);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1741;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_385);
         if(param1.flatId != this.var_385)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_385);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_528 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_528.procedure = this.onInputClick;
         this.var_1174 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1174);
         this.var_1174.procedure = this.onSelectTemplate;
         this.var_1106 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1647 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1646 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_35.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_35,WindowTracker.const_802,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_705 || this.var_528.text == "")
         {
            this.var_35.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_1106.isSelected);
         this.var_35.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1775,_loc2_,this.var_528.text,"",""));
         if(this.var_1647.isSelected || this.var_1646.isSelected || this.var_1106.isSelected)
         {
            this.var_35.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1647.isSelected,this.var_1646.isSelected,this.var_1106.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1818);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_2029);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_35.initMsg.roomMessageTemplates[this.var_1174.selection];
         if(_loc3_ != null)
         {
            this.var_705 = false;
            this.var_528.text = _loc3_;
         }
      }
   }
}
